	...
	public  StringBuffer readdata(String searchId)
	{
        ...
        while (it.hasNext())
	    {
			...
			if(pi!=null)
			{
                ...
                if (p.getProdName().toLowerCase().startsWith(searchId))
                {
                        sb.append("<product>");
                        sb.append("<id>" + p.getProdName() + "</id>");
                        sb.append("<productName>" + p.getProdName() + "</productName>");
                        sb.append("</product>");
                }
			}
       }
	...
	public static HashMap<String,Product> getData()
	{
		...
		try
		{
			...
			{	//Product p = new Product(rs.getString("Id"),rs.getString("productName"),rs.getDouble("productPrice"),rs.getString("productImage"),rs.getString("productManufacturer"),rs.getString("productCondition"),rs.getString("ProductType"),rs.getDouble("productDiscount"));
				Product p = new Product(rs.getString("prodName"),rs.getString("prodPrice"),
				...
				hm.put(rs.getString("prodName"), p);
			}
		}
		catch(Exception e)
		{
