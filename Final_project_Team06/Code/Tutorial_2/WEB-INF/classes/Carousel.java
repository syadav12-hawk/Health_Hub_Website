	...
	public String  carouselfeature(Utilities utility){
		...
		for(String user: prodRecmMap.keySet())
		{
			if(user.equals(utility.username()))
			{
				...
				sb.append("<a style='font-size: 24px;'>"+""+" Recommended Products and Services</a>");
				...
				*/
				sb.append("<div class='carousel slide' id='"+myCarousel+"' data-ride='carousel' style='width: 200%';>");
				...
				*/
				...
				for(String prod : productsList){
					...
					System.out.println("Without Trim "+prod);
					System.out.println("With Trim "+prod.trim());
					...
					Product prodObj = ProductRecommenderUtility.getProduct(prod.trim());
                    System.out.println("With Trim "+prodObj.getProdName());
                    System.out.println("With Trim "+prodObj.getProdPrice());
                    System.out.println("With Trim "+prodObj.getProdType());
                    System.out.println("With Trim "+prodObj.getProdName().toLowerCase()+"s");
					...
					sb.append("<h3>"+prodObj.getProdName()+"</h3>");
					System.out.println("recommender obj name is :" +prodObj.getProdName());
					sb.append("<strong>"+prodObj.getProdPrice()+"$</strong><ul>");
					sb.append("<li id='item'><img src='images/"+prodObj.getProdType().toLowerCase()+"s/"+prodObj.getImage()+"' alt='' /></li>");
					sb.append("<li><form method='post' action='Cart'>" +
							"<input type='hidden' name='name' value='"+prodObj.getProdName()+"'>"+
							"<input type='hidden' name='type' value='"+prodObj.getProdType()+"s'>"+
							"<input type='hidden' name='maker' value='"+prodObj.getProdRetailer()+"'>"+
							"<input type='hidden' name='access' value='"+" "+"'>"+
							"<input type='submit' class='btnbuy' value='Book Now'></form></li>");
					sb.append("<li><form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+prodObj.getProdName()+"'>"+
							"<input type='hidden' name='type' value='"+prodObj.getProdType()+"s'>"+
							"<input type='hidden' name='maker' value='"+prodObj.getProdRetailer()+"'>"+
							"<input type='hidden' name='access' value='"+" "+"'>"+
							"<input type='hidden' name='price' value='"+prodObj.getProdPrice()+"'>"+
							"<input type='submit' value='WriteReview' class='btnreview'></form></li>");
					sb.append("<li><form method='post' action='ViewReview'>"+"<input type='hidden' name='name' value='"+prodObj.getProdName()+"'>"+
							"<input type='hidden' name='type' value='"+prodObj.getProdType()+"s'>"+
							"<input type='hidden' name='maker' value='"+prodObj.getProdRetailer()+"'>"+
							"<input type='hidden' name='access' value='"+" "+"'>"+
				...
				*/
				sb.append("<a class='left carousel-control' href='#"+myCarousel+"' data-slide='prev' style = 'width : 1% ;background-color:#D7e4ef; opacity :1'>"+
						"<span class='glyphicon glyphicon-chevron-left' style = 'color :red'></span>"+
						"<span class='sr-only'>Previous</span>"+
						"</a>");
				sb.append("<a class='right carousel-control' href='#"+myCarousel+"' data-slide='next' style = 'width : 1% ;background-color:#D7e4ef; opacity :1'>"+
