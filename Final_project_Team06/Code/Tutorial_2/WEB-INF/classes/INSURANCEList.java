...
public class INSURANCEList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (String key : hm.keySet()) { 
			...
			if (product.getProdRetailer().trim().equalsIgnoreCase(CategoryName.trim())){
				System.out.println("Insidee Function");
				show_insurances(key,pw,hm);
		...
		*/
		...
		if (CategoryName == null)	
		{
			for (String key : hm.keySet()) { 
				...
				if (product.getProdType().equals("INSURANCE")){
					show_insurances(key,pw,hm);
					count=count+1;
				}
		    }
	...
	public void show_insurances(String name,PrintWriter pw,HashMap<String,Product> hm){
		Product product=hm.get(name);		
		
		pw.print("<td><div id='shop_item'>");
		pw.print("<h3>"+product.getProdName()+"</h3>");
		pw.print("<strong>$"+product.getProdPrice()+"</strong><ul>");
		pw.print("<li id='item'><img src='images/insurances/"+product.getImage()+"' alt='' /></li>");///////
		...
		pw.print("<li><form method='post' action='Cart'>" +
				...
				"<input type='hidden' name='type' value='Insurances'>"+
