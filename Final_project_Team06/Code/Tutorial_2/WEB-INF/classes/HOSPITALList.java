...
public class HOSPITALList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		pw.print("<a style='font-size: 24px;'>"+name+" Hospitals</a>");
		...
		*/
		...
		if (CategoryName == null)	
		{
			for (String key : hm.keySet()) { 
				...
				if (product.getProdType().equals("HOSPITAL")){
					show_hospitals(key,pw,hm);
					count=count+1;
				}
		    }
	...
	public void show_hospitals(String name,PrintWriter pw,HashMap<String,Product> hm){
		Product product=hm.get(name);		
		
		pw.print("<td><div id='shop_item'>");
		pw.print("<h3>"+product.getProdName()+"</h3>");
		pw.print("<strong>$"+product.getProdPrice()+"</strong><ul>");
		pw.print("<li id='item'><img src='images/hospitals/"+product.getImage()+"' alt='' /></li>");///////
